...
function SettingsPage() {
  ...
  async function handleCropAndUpload() {
    ...
    canvas.toBlob(async (blob) => {
      ...
      const urlWithCache = `${baseUrl}?t=${Date.now()}`
  ...
  async function handlePasswordSave(e) {
    ...
    const { error: signInError } = await supabase.auth.signInWithPassword({
      email: profile.email, password: passwordData.currentPassword,
    })
  ...
  function Avatar({ size = 'sm' }) {
    const isLg = size === 'lg'
    const sizeClass = isLg ? 'w-24 h-24 rounded-xl' : 'w-10 h-10 rounded-full'
    const iconSize = isLg ? 'h-12 w-12' : 'h-6 w-6'
    if (avatarUrl) {
      return <img src={avatarUrl} alt="Profile" className={`${sizeClass} object-cover flex-shrink-0`} onError={() => setAvatarUrl(null)} />
    }
    return (
      <div className={`${sizeClass} bg-gray-600 flex items-center justify-center flex-shrink-0`}>
        <svg xmlns="http://www.w3.org/2000/svg" className={`${iconSize} text-gray-300`} fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleCropAndUpload} disabled={avatarUploading || !completedCrop}
                  className={`px-5 py-2 rounded-xl text-sm font-semibold text-white transition
                    ${avatarUploading || !completedCrop ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
            ...
            <div className="flex-1 bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
                  ...
                  <form onSubmit={handleSave}>
                    ...
                    <div className="flex justify-end">
                      <button type="submit" disabled={saving}
                        className={`font-semibold px-8 py-3 rounded-xl transition text-sm
                          ${saving ? 'bg-blue-400 text-white cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
                    ...
                    <form onSubmit={handlePasswordSave} className="flex flex-col gap-4">
                      <div>
                        ...
                        <div className="relative">
                          <input type={showCurrent ? 'text' : 'password'} name="currentPassword"
                            ...
                            className="w-full bg-gray-100 text-gray-800 placeholder-gray-400 rounded-lg px-4 py-3 pr-12 text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                      ...
                      <div className="flex justify-end">
                        <button type="submit" disabled={passwordSaving}
                          className={`font-semibold px-8 py-3 rounded-xl transition text-sm
                            ${passwordSaving ? 'bg-blue-400 text-white cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
              ...
              {activeTab === 'notification' && (
                <div>
                  ...
                  <div className="flex flex-col gap-4">
                    {[
                      { key: 'newModules', label: 'New training modules available', desc: 'Get notified when new modules are added' },
                  ...
                  <div className="flex justify-end mt-6">
                    <button onClick={handleNotifSave} disabled={notifSaving}
                      className={`font-semibold px-8 py-3 rounded-xl transition text-sm
                        ${notifSaving ? 'bg-blue-400 text-white cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
              ...
              {activeTab === 'privacy' && (
                <div>
                  ...
                  <p className="text-gray-500 text-sm mb-6">Control your privacy and data settings</p>
                  {privacySuccess && (
                    <div className="bg-green-50 border border-green-300 rounded-xl px-4 py-3 mb-5">
                      <p className="text-green-600 text-sm font-semibold">✓ Privacy settings saved!</p>
                    </div>
                  )}
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 opacity-50">
                      <div>
                        <p className="text-gray-800 text-sm font-semibold">Show my progress to admin</p>
                        <p className="text-gray-400 text-xs">Allow your admin to view your training progress — this cannot be changed</p>
                      </div>
                    ...
                    {[
                      ...
                      { key: 'allowDataSharing', label: 'Allow data for improvement', desc: 'Help improve Averion by sharing anonymous usage data' },
                    ].map((item) => (
                      <div key={item.key} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                        <div>
                          <p className="text-gray-800 text-sm font-semibold">{item.label}</p>
                  ...
                  <div className="flex justify-end mt-6">
                    <button onClick={handlePrivacySave} disabled={privacySaving}
                      className={`font-semibold px-8 py-3 rounded-xl transition text-sm
                        ${privacySaving ? 'bg-blue-400 text-white cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}>
