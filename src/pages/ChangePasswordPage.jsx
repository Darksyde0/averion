...
function ChangePasswordPage() {
  ...
  function getStrength(pwd) {
    ...
    if (pwd.length < 4) return { score: 1, label: 'Too short', color: 'bg-red-500' }
    if (pwd.length < 6) return { score: 2, label: 'Weak', color: 'bg-orange-500' }
    if (pwd.length < 8) return { score: 3, label: 'Fair', color: 'bg-yellow-500' }
    if (pwd.length >= 8 && /[A-Z]/.test(pwd) && /[0-9]/.test(pwd)) return { score: 4, label: 'Strong', color: 'bg-green-500' }
      ...
      <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 rounded-full px-3 py-1 mb-6">
            ...
            <span className="text-yellow-400 text-xs font-semibold tracking-widest uppercase">First Time Setup</span>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
            ...
            <div className="flex flex-col gap-3">
              {[
                { icon: '✓', text: 'At least 8 characters long', ok: newPassword.length >= 8 },
                { icon: '✓', text: 'Contains an uppercase letter', ok: /[A-Z]/.test(newPassword) },
                { icon: '✓', text: 'Contains a number', ok: /[0-9]/.test(newPassword) },
                { icon: '✓', text: 'Contains a special character', ok: /[^A-Za-z0-9]/.test(newPassword) },
              ].map((tip, i) => (
        ...
        <div className="relative z-10 bg-white/5 border border-white/10 rounded-2xl p-5">
          ...
          <p className="text-gray-400 text-sm">Your administrator cannot see your password. Only you have access to it.</p>
        </div>
      </div>
        ...
        <div className="relative z-10 w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="lg:hidden bg-white/5 border border-white/10 rounded-xl p-4">
              <p className="text-gray-500 text-xs font-semibold uppercase tracking-wide mb-3">Password Tips</p>
              <div className="flex flex-col gap-2">
                {[
                  { text: 'At least 8 characters', ok: newPassword.length >= 8 },
                  { text: 'Contains an uppercase letter', ok: /[A-Z]/.test(newPassword) },
                  { text: 'Contains a number', ok: /[0-9]/.test(newPassword) },
                ].map((tip, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300
                      ${tip.ok ? 'bg-green-500/20' : 'bg-white/5'}`}>
                      <svg xmlns="http://www.w3.org/2000/svg"
                        className={`h-2.5 w-2.5 ${tip.ok ? 'text-green-400' : 'text-gray-600'}`}
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                      </svg>
                    </div>
          ...
          <div className="flex items-center gap-2 mt-6 justify-center">
            ...
            <p className="text-gray-600 text-xs">Your password is encrypted and never visible to anyone</p>
          </div>
