...
function UserProfile() {
    ...
    useEffect(() => { if (id && adminProfile?.id) fetchAll() }, [id, adminProfile])
    ...
    async function fetchAll() {
        ...
        const [userRes, simRes, progressRes, achieveRes, modulesRes] = await Promise.all([
            ...
            // ── Sim results: left join simulations so deleted sims still show ──
            ...
            // ── Module progress with module info ──
        ...
        // ── Assigned simulations (pending = not completed by user) ──
        // Fetch all non-expired visible sims for this org
        const now = new Date().toISOString()
        const { data: allOrgSims } = await supabase
            .from('simulations')
        ...
        return (
            <div className="flex min-h-screen bg-gray-50">
                ...
                <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarOpen ? 'ml-48' : 'ml-16'}`}>
                    ...
                    <div className="flex-1 flex items-center justify-center">
                        <div className="text-center">
                            <p className="text-gray-500 font-semibold mb-2">User not found</p>
    ...
    const avgScore = simResults.length > 0
        ? Math.round(simResults.reduce((sum, r) => sum + r.score, 0) / simResults.length)
        : null
