...
function RecentActivities() {
  ...
  async function fetchActivities() {
    ...
    const modGroupActivity = pendingModules.length > 0 ? {
      id: modGroupId,
      title: `${pendingModules.length} Module${pendingModules.length > 1 ? 's' : ''} Pending`,
      ...
      <div className="flex-1 flex flex-col p-3">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 py-8 gap-2">
            ...
            <p className="text-gray-500 text-xs font-semibold">No activity yet</p>
            ...
            {visible.map((activity, index) => {
              ...
              return (
                <div key={activity.id}
                  ...
                  className={`group flex items-center gap-3 px-2 py-3 rounded-xl transition-all duration-150 relative
                  ...
                  {/* ── Icon — neutral gray bg always ── */}
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-gray-100 
                    ${activity.type === 'simulation_done' ? 'text-blue-500'
                      : activity.type === 'module_done' ? 'text-green-500'
                      : activity.type === 'new_simulation' ? 'text-orange-500'
                      : 'text-purple-500'}`}>
                    {icons[activity.type]}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-gray-400 text-xs flex-shrink-0">{timeAgo(activity.time)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {activity.scoreValue !== null && (
                      <span className={`text-xs font-bold ${scoreColor(activity.scoreValue)}`}>
                        {activity.scoreValue}%
                      </span>
                    )}
                    ...
                    {/* Type pill — always shown, muted */}
                    <span className="text-gray-400 text-xs hidden group-hover:inline-block">
                    ...
                    {activity.action && (
                      <svg xmlns="http://www.w3.org/2000/svg"
                        className="h-3.5 w-3.5 text-gray-300 group-hover:text-gray-500 transition"
