...
function AdminTopBar({ onMenuClick }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`admin-notif-${profile.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'simulation_results' }, payload => {
        const r = payload.new
        ...
        addNotification({
          ...
          title: r.score < 50 ? `${userName} is at risk` : `${userName} completed a simulation`,
          subtitle: r.score < 50 ? `Scored ${r.score}% — below the 50% threshold` : `Scored ${r.score}%`,
          time: r.completed_at || new Date().toISOString(),
          score: r.score, read: false,
        })
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'module_progress' }, payload => {
        const p = payload.new
        ...
        addNotification({
          ...
          time: p.completed_at || new Date().toISOString(), read: false,
        })
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'users', filter: `organization_id=eq.${profile.id}` }, payload => {
        const u = payload.new
        ...
        addNotification({
          ...
          time: u.created_at || new Date().toISOString(), read: false,
        })
      })
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'achievements' }, payload => {
        ...
        addNotification({
          ...
          time: a.earned_at || new Date().toISOString(), read: false,
        })
      })
      .subscribe()
  ...
  async function fetchNotifications() {
    ...
    const userRes = await supabase.from('users').select('id, full_name, avatar_url')
  ...
  return (
    <div className="bg-[#0d1117] border-b border-white border-opacity-5 flex items-center justify-between px-6 h-12 flex-shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={notifRef}>
          ...
          {/* Dropdown */}
          {notifOpen && (
            <div className="absolute right-0 mt-2 w-88 bg-white rounded-xl shadow-2xl z-50 overflow-hidden border border-gray-100"
              style={{ width: '360px' }}>
              ...
              <div className="px-4 py-3.5 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-gray-800 text-sm font-semibold">Notifications</p>
                  ...
                  <p className="text-gray-400 text-xs mt-0.5">
                    {notifications.length > 0 ? `${notifications.length} recent activities` : 'No notifications yet'}
                  </p>
                </div>
                {notifications.some(n => !n.read) && (
                  <button onClick={markAllRead} className="text-blue-500 hover:text-blue-700 text-xs font-medium transition">
                    Mark all read
                  </button>
                )}
              </div>
                ...
                {notifLoading ? (
                  ...
                  <div className="p-2">
                    {notifications.map(notif => (
                      <div key={notif.id}
                        className={`flex items-start gap-3 px-3 py-2.5 rounded-lg transition hover:bg-gray-50 cursor-default
                          ${!notif.read ? 'bg-blue-50/40' : ''}`}>
                        <NotifIcon type={notif.type} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-gray-700 text-xs font-medium leading-snug">{notif.title}</p>
                            {!notif.read && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0 mt-1" />}
                          </div>
                          <p className="text-gray-400 text-xs mt-0.5 truncate">
                            {notif.subtitle}
                            {notif.score !== undefined && (
                              <span className={`ml-1 font-semibold ${getScoreColor(notif.score)}`}>· {notif.score}%</span>
                            )}
                          </p>
                          <p className="text-gray-300 text-xs mt-0.5">{timeAgo(notif.time)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {notifications.length > 0 && (
                <div className="px-4 py-2.5 border-t border-gray-100 bg-gray-50 flex items-center justify-center gap-1.5">
                  ...
                  <p className="text-gray-400 text-xs">Live · {notifications.length} activities</p>
                </div>
              )}
            </div>
          )}
